...
/// 显示背景图片
@property (nonatomic, strong) UIImageView *backgrondImgView;
                                   ...
                                   backgroundImg:(nullable UIImage *)backgroundImg
{
    ...
    bulletScreenView.backgroundImg = backgroundImg;
...
- (void)setupView {
    ...
    if (self.backgroundImg) {
        // 添加 Icon 视图
        CGRect iconFrame = CGRectMake(0, 0, self.contentWidth + 2 * self.contentX, self.contentHeight);
        ...
        [self.contentLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.center.equalTo(self.backgrondImgView);
        }];
    } else {
...
- (void)startLoopAnimation {
    ...
    if (0 == self.animationDuration) {
        ...
        if (self.backgroundImg) {
            self.backgrondImgView.frame = CGRectMake(0, 0, self.viewWidth, frame.size.height);
        }
