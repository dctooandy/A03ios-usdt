...
/// 停止
- (void)pingStop{
    MyLog(@"Ping 域名 [%@] 停止。",self.hostName);
    [self CleanAll:NXPingStatusDidFinished];
}

/// 超时
- (void)pingTimeout{
    MyLog(@"Ping 域名 [%@] 超时！",self.hostName);
...
/// 失败
- (void)pingFail{
    MyLog(@"Ping 域名 [%@] 失败！",self.hostName);
...
- (void)simplePing:(SimplePing *)pinger didStartWithAddress:(NSData *)address{
    MyLog(@"Ping 域名 [%@] 开始..",self.hostName);
    [self sendPing];
...
- (void)simplePing:(SimplePing *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber{
    MyLog(@"[%@] 发包<%hu> [成功]", self.hostName, sequenceNumber);
...
- (void)simplePing:(SimplePing *)pinger didReceivePingResponsePacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber{
    ...
    MyLog(@"[%@] 接收到<%hu> [成功]: size=%ldbit, time=%.3fms", self.hostName, sequenceNumber, packet.length, timeMilliseconds);
...
- (void)simplePing:(SimplePing *)pinger didFailWithError:(NSError *)error{
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(pingTimeout) object:nil];
    MyLog(@"[%@] PING【ERROR: %@】[失败]", self.hostName, error.localizedDescription);
    [self pingFail];
}
...
- (void)simplePing:(SimplePing *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error{
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(pingTimeout) object:nil];
    MyLog(@"[%@] 发包<%hu>【ERROR: %@】[失败]", self.hostName, sequenceNumber, error.localizedDescription);
...
- (void)simplePing:(SimplePing *)pinger didReceiveUnexpectedPacket:(NSData *)packet{
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(pingTimeout) object:nil];
//    MyLog(@"[%@] 接收到<异常包> [丢弃]", self.hostName);
