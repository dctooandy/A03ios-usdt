...
+ (void)pingFastestHost:(nonnull NSArray *)hostList handler:(void (^)(NSString *host))handler{
    if (hostList.count == 0) {
        MyLog(@"hostList不可为空");
        handler(nil);
        return ;
    ...
    dispatch_group_notify(group, dispatch_get_main_queue(), ^{
        ...
        MyLog(@"❀~完结撒花~❀ Ping最快的地址是：%@, 平均延迟：%.3lfms", fastestHost, minAvg);
        handler(fastestHost);
    });
}
...
+ (void)pingFastestHost:(NSArray *)hostList progress:(void (^)(CGFloat))progress handler:(void (^)(NSString *))handler{
    ...
    dispatch_group_notify(group, dispatch_get_main_queue(), ^{
        ...
        // 0. 数据验证
        for (NSString *host in failHostList) {
            MyLog(@"ping失败(Error)的域名: [%@]", host);
            [pingResult removeObjectForKey:host];
        }
        for (NSString *host in pingResult.allKeys) {
            if (pingResult[host].count == 0) {
                MyLog(@"ping异常(100%%返回异常包)的域名: [%@]", host);
                [pingResult removeObjectForKey:host];
            }
        }
        // 无可用数据: 证明所有域名都出现错误/异常
        if (pingResult.count == 0) {
            MyLog(@"完成步数%f / 总步数%f", rateCount, totalCount);
            MyLog(@" ❀-- 完结：所有域名都出现错误/异常，默认不切换域名 --❀");
            progress(1.0);
            handler(nil);
            return ;
        }
        ...
        for (NSString * host in pingResult.allKeys) {
            ...
            MyLog(@"[%@] 平均延迟 >>> %.3lfms", fHost, avg);
        }
        
        MyLog(@"完成步数%f / 总步数%f", rateCount, totalCount);
        // 1.1 所有域名都超时
        if (minAvg == 1000 || [fastestHost isEqualToString:@""]) {
            MyLog(@" ❀-- 完结：所有可用域名都超时，默认不切换域名 --❀");
            progress(1.0);
            handler(fastestHost);
