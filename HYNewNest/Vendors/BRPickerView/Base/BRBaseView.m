...
- (UIView *)alertView {
    if (!_alertView) {
        ...
        _alertView.backgroundColor = BR_RGB_HEX(0x212137, 1.0);
        // 设置子视图的大小随着父视图变化
        ...
        UIBezierPath *bezierPath = [UIBezierPath bezierPathWithRoundedRect:_alertView.bounds byRoundingCorners:rectCorner cornerRadii:CGSizeMake(20, 20)];
...
- (UIButton *)leftBtn {
    if (!_leftBtn) {
        ...
        _leftBtn.frame = CGRectMake(0, 15, 60, 30);
...
//        [_leftBtn setTitle:@"取消" forState:UIControlStateNormal];
        [_leftBtn setImage:[UIImage imageNamed:@"icon_close"] forState:UIControlStateNormal];
...
- (UIButton *)rightBtn {
    if (!_rightBtn) {
        ...
        _rightBtn.frame = CGRectMake(self.alertView.frame.size.width - 65, 15, 60, 30);
        ...
        _rightBtn.titleLabel.font = [UIFont systemFontOfSize:16.0f * kScaleFit weight:UIFontWeightMedium];
        [_rightBtn setTitleColor:BR_RGB_HEX(0x6666FF, 1.0) forState:UIControlStateNormal];
//        [_rightBtn setTitle:@"完成" forState:UIControlStateNormal];
        [_rightBtn setImage:[UIImage imageNamed:@"l_close"] forState:UIControlStateNormal];
...
- (UIView *)lineView {
    if (!_lineView) {
        _lineView = [[UIView alloc]initWithFrame:CGRectMake(0, kTopViewHeight, self.alertView.frame.size.width, 1)];
        _lineView.backgroundColor = kBRLineColor;
