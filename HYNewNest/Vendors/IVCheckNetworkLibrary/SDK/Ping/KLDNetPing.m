...
#import "KLDNetPing.h"
#import "KLDNetTimer.h"

#define MAXCOUNT_PING 4

@interface KLDNetPing () {
...
@property (nonatomic, strong, readwrite) KLDSimplePing *pinger;

@end


@implementation KLDNetPing
...
- (void)runWithHostName:(NSString *)hostName normalPing:(BOOL)normalPing{
    ...
    self.pinger = [[KLDSimplePing alloc] initWithHostName:hostName];
    assert(self.pinger != nil);
...
- (void)sendPing
{
    ...
    if (_sendCount > MAXCOUNT_PING) {
        ...
        assert(self.pinger != nil);
        _sendCount++;
        _startTime = [KLDNetTimer getMicroSeconds];
        if (_isLargePing) {
            NSString *testStr = @"";
            for (int i=0; i<408; i++) {
...
- (void)pingTimeout:(NSTimer *)index
{
    if ([[index userInfo] intValue] == _sendCount && _sendCount <= MAXCOUNT_PING + 1 &&
        ...
        if (_hostAddress && _hostAddress.length > 4) {
            NSRange range = NSMakeRange(1, _hostAddress.length - 3);
            securiteIp = [NSString stringWithFormat:@"*%@*",[_hostAddress substringWithRange:range]];
        }
        NSString *timeoutLog =
            [NSString stringWithFormat:@"ping: cannot resolve %@: TimeOut", securiteIp];
...
#pragma unused(packet)
    ...
    NSString *icmpReplyType = [NSString stringWithFormat:@"%@", [KLDSimplePing icmpInPacket:packet]->type == 129 ? @"KICMPv6TypeEchoReply" : @"KICMPv4TypeEchoReply"];
    long responseSecond = [KLDNetTimer computeDurationSince:_startTime] / 1000;
    NSString *securiteIp = nil;
    if (_hostAddress && _hostAddress.length > 4) {
        NSRange range = NSMakeRange(1, _hostAddress.length - 3);
        securiteIp = [NSString stringWithFormat:@"*%@*",[_hostAddress substringWithRange:range]];
    ...
    NSString *successLog = [NSString
        stringWithFormat:@"%lu bytes from %@ icmp_seq=#%u type=%@ time=%ldms",
                         (unsigned long)[packet length], securiteIp, sequenceNumber, icmpReplyType, responseSecond];
    //记录ping成功的数据
    if (self.delegate && [self.delegate respondsToSelector:@selector(appendPingLog:)]) {
...
{
    const KICMPHeader *icmpPtr;
    if (self.pinger && pinger == self.pinger) {
        icmpPtr = [KLDSimplePing icmpInPacket:packet];
        NSString *errorLog = @"";
        if (icmpPtr != NULL) {
            errorLog = [NSString
