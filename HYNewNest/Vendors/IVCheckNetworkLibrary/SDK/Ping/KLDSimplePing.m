...
- (void)didFailWithError:(NSError *)error {
    id<KLDSimplePingDelegate>  strongDelegate;
...
- (void)sendPingWithData:(NSData *)data {
    ...
    if (payload == nil) {
        ...
        // Our dummy payload is sized so that the resulting ICMP packet, including the KICMPHeader, is
...
+ (NSUInteger)KICMPHeaderOffsetInIPv4Packet:(NSData *)packet {
 ...
 *
 *      * It needs to zero out the `checksum` field of the KICMPHeader in order to do
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    KICMPHeaderOffset = [[self class] KICMPHeaderOffsetInIPv4Packet:packet];
    if (KICMPHeaderOffset != NSNotFound) {
        icmpPtr = (struct KICMPHeader *) (((uint8_t *) packet.mutableBytes) + KICMPHeaderOffset);
        
        receivedChecksum   = icmpPtr->checksum;
        icmpPtr->checksum  = 0;
        calculatedChecksum = in_cksum(icmpPtr, packet.length - KICMPHeaderOffset);
            ...
            if ( (icmpPtr->type == KICMPv4TypeEchoReply) && (icmpPtr->code == 0) ) {
                if ( OSSwapBigToHostInt16(icmpPtr->identifier) == self.identifier ) {
                    ...
                    if ([self validateSequenceNumber:sequenceNumber]) {
                        ...
                        [packet replaceBytesInRange:NSMakeRange(0, KICMPHeaderOffset) withBytes:NULL length:0];
...
static void SocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    obj = (__bridge KLDSimplePing *) info;
    assert([obj isKindOfClass:[KLDSimplePing class]]);
        ...
        for (NSData * address in addresses) {
            ...
            if ( address.length >= sizeof(struct sockaddr) ) {
                switch (addrPtr->sa_family) {
                    case AF_INET: {
                        if (self.addressStyle != KSimplePingAddressStyleICMPv6) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
                    case AF_INET6: {
                        if (self.addressStyle != KSimplePingAddressStyleICMPv4) {
                            self.hostAddress = address;
                            resolved = true;
                        }
                    } break;
...
{
    const struct KICMPHeader *result;
    result = nil;
