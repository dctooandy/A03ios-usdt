...
/*
 * @protocal KLDNetTraceRouteDelegate监测TraceRoute命令的的输出到日志变量；
 *
 */
@protocol KLDNetTraceRouteDelegate <NSObject>
...
/*
 * @class KLDNetTraceRoute TraceRoute网络监控
 * 主要是通过模拟shell命令traceRoute的过程，监控网络站点间的跳转
 * 默认执行20转，每转进行三次发送测速
 */
@interface KLDNetTraceRoute : NSObject {
    int udpPort;      //执行端口
    int maxTTL;       //执行转数
...
@property (nonatomic, weak) id<KLDNetTraceRouteDelegate> delegate;
