...
@implementation KLDNetTraceRoute

/**
 * 初始化
 */
- (KLDNetTraceRoute *)initWithMaxTTL:(int)ttl
                            timeout:(int)timeout
...
- (Boolean)doTraceRoute:(NSString *)host
{
    ...
    NSArray *serverDNSs = [KLDNetGetAddress getDNSsWithDormain:host];
    ...
    while (ttl <= maxTTL) {
        ...
        for (int try = 0; try < maxAttempts; try ++) {
            startTime = [KLDNetTimer getMicroSeconds];
            //发送成功返回值等于发送消息的长度
            ...
            if (FD_ISSET(recv_sock, &readfds) > 0) {
                ...
                if ((res = recvfrom(recv_sock, buf, 100, 0, (struct sockaddr *)&fromAddr, &n)) <
                    ...
                    icmp = true;
                    delta = [KLDNetTimer computeDurationSince:startTime];
