...
/*
 * @protocal KLDNetPingDelegate监测Ping命令的的输出到日志变量；
 *
 */
@protocol KLDNetPingDelegate <NSObject>
@optional
...
/*
 * @class KLDNetPing ping监控
 * 主要是通过模拟shell命令ping的过程，监控目标主机是否连通
 * 连续执行五次，因为每次的速度不一致，可以观察其平均速度来判断网络情况
 */
@protocol KLDSimplePingDelegate;
@interface KLDNetPing : NSObject <KLDSimplePingDelegate> {
}

@property (nonatomic, weak, readwrite) id<KLDNetPingDelegate> delegate;
