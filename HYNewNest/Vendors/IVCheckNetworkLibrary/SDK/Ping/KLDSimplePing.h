...
@protocol KLDSimplePingDelegate;

/*! Controls the IP address version used by SimplePing instances.
 */

typedef NS_ENUM(NSInteger, KSimplePingAddressStyle) {
    KSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    KSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    KSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};


@interface KLDSimplePing : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<KLDSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) KSimplePingAddressStyle addressStyle;
...
+ (const struct KICMPHeader *)icmpInPacket:(NSData *)packet;

@end



@protocol KLDSimplePingDelegate <NSObject>
...
- (void)simplePing:(KLDSimplePing *)pinger didStartWithAddress:(NSData *)address;
 ...
 */
- (void)simplePing:(KLDSimplePing *)pinger didFailWithError:(NSError *)error;
 ...
 *      considered API.
 *  \param pinger The object issuing the callback.
 *  \param packet The packet that was sent; this includes the ICMP header (`KICMPHeader`) and the
...
- (void)simplePing:(KLDSimplePing *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;
 ...
 *  \param error Describes the failure.
 */
- (void)simplePing:(KLDSimplePing *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error;
 ...
 *  \param pinger The object issuing the callback.
 *  \param packet The packet received; this includes the ICMP header (`KICMPHeader`) and any data that
 *      follows that in the ICMP message but does not include any IP-level headers.
 *  \param sequenceNumber The ICMP sequence number of that packet.
 */
- (void)simplePing:(KLDSimplePing *)pinger didReceivePingResponsePacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;
 ...
 *      follows that in the ICMP message but does not include any IP-level headers.
 */
- (void)simplePing:(KLDSimplePing *)pinger didReceiveUnexpectedPacket:(NSData *)packet;
...
typedef struct KICMPHeader KICMPHeader;

__Check_Compile_Time(sizeof(KICMPHeader) == 8);
__Check_Compile_Time(offsetof(KICMPHeader, type) == 0);
__Check_Compile_Time(offsetof(KICMPHeader, code) == 1);
__Check_Compile_Time(offsetof(KICMPHeader, checksum) == 2);
__Check_Compile_Time(offsetof(KICMPHeader, identifier) == 4);
__Check_Compile_Time(offsetof(KICMPHeader, sequenceNumber) == 6);

enum {
    KICMPv4TypeEchoRequest = 8,          ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    KICMPv4TypeEchoReply   = 0           ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};

enum {
    KICMPv6TypeEchoRequest = 128,        ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    KICMPv6TypeEchoReply   = 129         ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};
