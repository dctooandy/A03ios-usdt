...
#import "KLDNetDiagnoService.h"
#import "KLDNetPing.h"
#import "KLDNetTraceRoute.h"
#import "KLDNetGetAddress.h"
#import "KLDNetTimer.h"
#import "KLDNetConnect.h"

static NSString *const kPingOpenServerIP = @"";
static NSString *const kCheckOutIPURL = @"";

@interface KLDNetDiagnoService () <KLDNetPingDelegate, KLDNetTraceRouteDelegate,
                                  KLDNetConnectDelegate> {
    ...
    KLDNetPing *_netPinger;
    KLDNetTraceRoute *_traceRouter;
    KLDNetConnect *_netConnect;
}

@end

@implementation KLDNetDiagnoService
...
//        [self recordOutIPInfo];
    ...
    if (_isRunning) {
        ...
        if ([_hostAddress count] > 0) {
            _netConnect = [[KLDNetConnect alloc] init];
    ...
    if (_isRunning) {
        //开始诊断traceRoute
        [self recordStepInfo:@"\n开始traceroute..."];
        _traceRouter = [[KLDNetTraceRoute alloc] initWithMaxTTL:TRACEROUTE_MAX_TTL
...
- (void)recordLocalNetEnvironment
{
    ...
    _curNetType = [KLDNetGetAddress getNetworkTypeFromStatusBar];
    if (_curNetType == 0) {
    ...
    //本地ip信息
    _localIp = [KLDNetGetAddress deviceIPAdress];
    [self recordStepInfo:[NSString stringWithFormat:@"当前本机IP: %@", _localIp]];

    if (_curNetType == KNETWORK_TYPE_WIFI) {
        _gatewayIp = [KLDNetGetAddress getGatewayIPAddress];
    ...
    _dnsServers = [NSArray arrayWithArray:[KLDNetGetAddress outPutDNSServers]];
    ...
    // host地址IP列表
    long time_start = [KLDNetTimer getMicroSeconds];
    _hostAddress = [NSArray arrayWithArray:[KLDNetGetAddress getDNSsWithDormain:_dormain]];
    long time_duration = [KLDNetTimer computeDurationSince:time_start] / 1000;
    if ([_hostAddress count] == 0) {
        [self recordStepInfo:[NSString stringWithFormat:@"DNS解析结果: 解析失败"]];
    } else {
        [self recordStepInfo:[NSString stringWithFormat:@"DNS解析结果: %@ (%ldms)",
...
- (void)pingDialogsis:(BOOL)pingLocal
{
    ...
    [self recordStepInfo:@"\n开始ping..."];
    _netPinger = [[KLDNetPing alloc] init];
