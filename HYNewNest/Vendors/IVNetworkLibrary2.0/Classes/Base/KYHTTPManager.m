...
- (NSURLSessionTask *)sendRequestWithUrl:(NSString *)url parameters:(id)parameters callBack:(KYHTTPCallBack)callBack
{
    return [self sendRequestWithUrl:url
                   ...
                   failure:(void (^)(NSURLSessionDataTask * _Nullable, NSError * _Nonnull))failure
{
    ...
    switch (self.method) {
        ...
        case KYHTTPMethodHEAD: {
            task = [self.sessionManager HEAD:fullUrl parameters:parameters success:^(NSURLSessionDataTask * _Nonnull task) {
                if (success) {
                    success(task,nil);
                }
            } failure:failure];
